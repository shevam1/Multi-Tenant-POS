...
function BarChart({ rows, max, fmtVal, color = 'bg-primary' }: {
      ...
      {rows.map((r, i) => (
        <div key={i} className="flex items-center gap-3 text-sm">
          ...
          <div className="h-5 flex-1 rounded bg-secondary">
            <div className={`flex h-5 items-center justify-end rounded ${color} px-2`} style={{ width: `${Math.max(4, Math.round(r.value / m * 100))}%` }}>
              <span className="whitespace-nowrap text-xs font-medium text-white">{fmtVal(r.value)}</span>
            </div>
          </div>
        </div>
      ))}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="break-inside-avoid rounded-xl border bg-card p-5 shadow-sm">
...
export default function AnalyticsPage() {
  const { me } = useShell();
  const allowed = me.permissions.includes('analytics.view');
  ...
  function applyPeriodPreset(p: 'day' | 'week' | 'month' | 'year') {
    ...
    if (p === 'day') start.setDate(today.getDate() - 13);
    else if (p === 'week') start.setDate(today.getDate() - 7 * 11);
    else if (p === 'month') start.setMonth(today.getMonth() - 11);
    else start.setFullYear(today.getFullYear() - 4);
  ...
  function toggleSection(key: string) { setEnabled(s => { const n = new Set(s); n.has(key) ? n.delete(key) : n.add(key); return n; }); }

  const on = (k: string) => enabled.has(k);
  const r = report;

  if (!allowed) return <div className="mx-auto max-w-7xl px-8 py-8 text-sm text-muted-foreground">You don&apos;t have access to analytics.</div>;
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-5 px-8 py-8">
      ...
      <div className="no-print flex flex-wrap items-center gap-3">
        <h1 className="mr-2 text-3xl font-bold tracking-tight">HQ Analytics</h1>
        <div className="ml-auto flex flex-wrap items-center gap-2">
          <div className="flex overflow-hidden rounded-lg border">
            {(['day', 'week', 'month', 'year'] as const).map(p => (
              <button key={p} onClick={() => applyPeriodPreset(p)}
                className={`px-3 py-1.5 text-xs font-medium capitalize ${period === p ? 'bg-primary text-primary-foreground' : 'bg-white hover:bg-secondary'}`}>
          ...
          <select className="rounded border bg-white px-2 py-1.5 text-xs" value={storeId} onChange={e => setStoreId(e.target.value)}>
            <option value="">All locations</option>
            {stores.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
          </select>
          <select className="rounded border bg-white px-2 py-1.5 text-xs" value={groomerId} onChange={e => setGroomerId(e.target.value)}>
            <option value="">All groomers</option>
            {groomers.map(g => <option key={g.id} value={g.id}>{g.fullName}</option>)}
          </select>
          <div className="relative">
            <button onClick={() => setShowSections(s => !s)} className="rounded border px-3 py-1.5 text-xs font-medium hover:bg-secondary">Sections ({enabled.size})</button>
            {showSections && (
              <div className="absolute right-0 z-20 mt-1 w-60 rounded-xl border bg-white p-3 shadow-xl">
                <div className="mb-2 flex justify-between">
                  <button onClick={() => setEnabled(new Set(SECTIONS.map(s => s.key)))} className="text-xs text-primary">Select All</button>
                  <button onClick={() => setEnabled(new Set())} className="text-xs text-primary">Deselect All</button>
                </div>
                <div className="max-h-72 space-y-1 overflow-y-auto">
                  {SECTIONS.map(s => (
                    <label key={s.key} className="flex cursor-pointer items-center gap-2 py-0.5 text-sm">
                      <input type="checkbox" checked={on(s.key)} onChange={() => toggleSection(s.key)} />{s.label}
                    </label>
                  ))}
                </div>
                <button onClick={() => setShowSections(false)} className="mt-2 w-full rounded-md bg-primary py-1.5 text-xs font-medium text-primary-foreground">Apply</button>
              </div>
            )}
          </div>
          ...
          <button onClick={() => window.print()} className="rounded-md bg-amber-accent px-3 py-1.5 text-xs font-bold text-neutral-900">Download PDF</button>
        </div>
      </div>
            ...
            <Section title="Sales Item">
              <div className="overflow-hidden rounded-lg border">
                <table className="w-full text-sm">
                  <thead className="bg-secondary text-xs uppercase text-muted-foreground"><tr>{['Item', 'Count', 'Revenue'].map(h => <th key={h} className="px-4 py-2 text-left">{h}</th>)}</tr></thead>
                  <tbody className="divide-y">
                    ...
                    {r.salesItems.length === 0 && <tr><td colSpan={3} className="px-4 py-4 text-center text-muted-foreground">No sales.</td></tr>}
                  </tbody>
                </table>
              </div>
            </Section>
              ...
              <Section title="Bookings by Status">
                <div className="space-y-1 text-sm">
                  {Object.entries(r.bookingsByStatus).map(([k, v]) => (
                    <div key={k} className="flex justify-between"><span className="text-muted-foreground">{k.replace(/_/g, ' ')}</span><span className="font-medium">{v}</span></div>
                  ))}
                </div>
              </Section>
            )}
            ...
            {on('rates') && (
              <Section title="No-show / Cancellation">
                <div className="grid grid-cols-2 gap-4">
                  <div><p className="text-xs uppercase text-muted-foreground">No-show rate</p><p className={`text-2xl font-bold ${r.rates.noShowRate > 10 ? 'text-amber-600' : ''}`}>{r.rates.noShowRate}%</p></div>
                  <div><p className="text-xs uppercase text-muted-foreground">Cancellation rate</p><p className="text-2xl font-bold">{r.rates.cancellationRate}%</p></div>
                </div>
              </Section>
            )}
          </div>
